...
history=vae.fit([train_flat_features,ps_train_flat_onehot_label],train_flat_pd_onehot_labels, epochs=150, batch_size=256, validation_data=([test_flat_features,ps_test_flat_onehot_label],test_flat_pd_onehot_labels))
...
for i ,j in enumerate(pr_s):
    temp=[]
    idx=train_new_data.loc[:,'tumor_type']==j   ### tumor_type or primary_site
    for ff in train_new_data.loc[idx,'barcode_file']:
        temp.append(read_features(ff))
    
    #print(np.array(flat_temp).shape)
    features_dic[j]=temp
...
for i,j in enumerate(pr_s): 
    ...
    for ii in range(len(features_dic[j])):
        ...
        label=enc.transform([le.transform([j])])*np.ones((tempo.shape[0],1),dtype=np.float16)                
...
# fisher_feature2=fisher_feature.copy()
...
# prediction_correct = []    
# # recall calculation
# for i, ps in enumerate(new_data.primary_site_x.unique().tolist()):
#     prediction_correct.append(ps in winners[i])
